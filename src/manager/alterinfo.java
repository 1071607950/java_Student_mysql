...
import main.Jdbc;
import main.Student;
public class alterinfo extends JPanel{
	public alterinfo(){
		this.setLayout(null);
        ...
        Student.button1.addActionListener(new ActionListener(){
        	public void actionPerformed(ActionEvent e){
        		if(!Student.tnumber.getText().matches("^\\d{1,10}$")) {
            		...
            		JOptionPane.showMessageDialog(null,"姓名长度在1~20之间，只能包含中文、英文、数字！","修改",JOptionPane.WARNING_MESSAGE);
            		return;
        			}
        		else if(!Student.tgender.getText().matches("^\\u7537|\\u5973$")) {
            		JOptionPane.showMessageDialog(null,"性别只能为 男或女！","修改",JOptionPane.WARNING_MESSAGE);
            		return;
        			}
        		else if(!Student.tnation.getText().matches("^[\\u4e00-\\u9fa5]{1,2}$")) {
                	JOptionPane.showMessageDialog(null,"民族长度在1~2之间，只能包含中文！","修改",JOptionPane.WARNING_MESSAGE);
                	return;
        			}
                else if(!Student.tbloodtype.getText().matches("^[A-Za-z]{1,2}$")) {
                    JOptionPane.showMessageDialog(null,"血型长度在1~2之间，只能包含字母！","修改",JOptionPane.WARNING_MESSAGE);
                    return;
        			}
                else if(!Student.tbirthday.getText().matches("^\\d{4}-\\d{1,2}-\\d{1,2}")) {
                    JOptionPane.showMessageDialog(null,"出生日期格式为 1900-01-01！","修改",JOptionPane.WARNING_MESSAGE);
        			return;
        			}
                else if(!Student.tyxname.getText().matches("^[\\u4E00-\\u9FA5A-Za-z0-9]{1,20}$")) {
                    JOptionPane.showMessageDialog(null,"院系长度在1~20之间，只能包含中文、英文、数字" , "修改",JOptionPane.WARNING_MESSAGE);
                    return;
        		}
                else if(!Student.tzyname.getText().matches("^[\\u4E00-\\u9FA5A-Za-z0-9]{1,20}$")) {
                    JOptionPane.showMessageDialog(null,"专业长度在1~20之间，只能包含中文、英文、数字！","修改",JOptionPane.WARNING_MESSAGE);
                    return;
        		}
                else if(!Student.tbjname.getText().matches("^[\\u4E00-\\u9FA5A-Za-z0-9]{1,20}$")) {
                    JOptionPane.showMessageDialog(null,"班级长度在1~20之间，只能包含中文、英文、数字！","修改",JOptionPane.WARNING_MESSAGE);
                    return;
        		}
        		else{
                	//调用Jdbc
        			try {
        				Jdbc.update("update students set"
    							+" name='"+Student.tname.getText().toString().trim()
    							+"' ,gender='"+Student.tgender.getText().toString().trim()
    							+"' ,nation='"+Student.tnation.getText().toString().trim()
    							+"' ,bloodtype='"+Student.tbloodtype.getText().toString().trim()
    							+"' ,birthday='"+Student.tbirthday.getText().toString().trim()
    							+"' ,yxname='"+Student.tyxname.getText().toString().trim()
    							+"' ,zyname='"+Student.tzyname.getText().toString().trim()
    							+"' ,bjname='"+Student.tbjname.getText().toString().trim()
    							+"' where number='"+Student.tnumber.getText().toString().trim()
    							+"'"
    							);
                	}catch(Exception e1) {
            			e1.printStackTrace();
