...
import main.Jdbc;
import main.Student;
public class addinfo extends JPanel{
	public addinfo(){
		this.setLayout(null);
        ...
        Student.button1.addActionListener(new ActionListener(){
        	public void actionPerformed(ActionEvent e){
        		if(!Student.tnumber.getText().matches("^\\d{1,10}$")) {
                	...
                	//调用Jdbc
                	try {
                		Jdbc.insert("insert into students values('"
            					+Student.tnumber.getText().toString().trim()
            					+"','"+Student.tname.getText().toString().trim()
            					+"','"+Student.tgender.getText().toString().trim()
            					+"','"+Student.tnation.getText().toString().trim()
            					+"','"+Student.tbloodtype.getText().toString().trim()
            					+"','"+Student.tbirthday.getText().toString().trim()
            					+"','"+Student.tyxname.getText().toString().trim()
            					+"','"+Student.tzyname.getText().toString().trim()
            					+"','"+Student.tbjname.getText().toString().trim()
            					+"')");
                	}catch(SQLIntegrityConstraintViolationException a){
                		JOptionPane.showMessageDialog(null,"学号不能重复！","添加",JOptionPane.ERROR_MESSAGE);
                	}catch(Exception e1) {
            			e1.printStackTrace();
                	}
